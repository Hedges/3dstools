...
int EsTicket::CreateTicket(const char* signature_issuer, const u8 rsa_modulus[Crypto::kRsa2048Size], const u8 rsa_priv_exponent[Crypto::kRsa2048Size])
{
	...
	memset(body_.ecc_public_key, 0, kEccPublicKeyLen);
	...
	safe_call(ticket_.alloc(EsSign::kRsa2048SignLen + sizeof(sTicketBody) + be_word(index_ctrl_hdr.total_size)));

	// copy data into blob
	memcpy(ticket_.data() + EsSign::kRsa2048SignLen, (const u8*)&body_, sizeof(struct sTicketBody));
	memcpy(ticket_.data() + EsSign::kRsa2048SignLen + sizeof(struct sTicketBody), (const u8*)&index_ctrl_hdr, sizeof(struct sContentMaskHeader));
	
	struct sContentMaskChunk* index_entries = (struct sContentMaskChunk*)(ticket_.data() + EsSign::kRsa2048SignLen + sizeof(struct sTicketBody) + sizeof(struct sContentMaskHeader));
	...
	Crypto::Sha256(ticket_.data() + EsSign::kRsa2048SignLen, ticket_.size() - EsSign::kRsa2048SignLen, hash);
	if (EsSign::RsaSign(EsSign::ES_SIGN_RSA2048_SHA256, hash, rsa_modulus, rsa_priv_exponent, ticket_.data()) != 0)
	{
