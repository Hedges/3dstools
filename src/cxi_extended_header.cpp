...
int CxiExtendedHeader::CreateExheader(const u8 ncch_rsa_modulus[Crypto::kRsa2048Size], const u8 accessdesc_rsa_modulus[Crypto::kRsa2048Size], const u8 accessdesc_rsa_priv_exponent[Crypto::kRsa2048Size])
{
	...
	if (ncch_rsa_modulus)
	{
		memcpy(access_descriptor_.ncch_rsa_modulus, ncch_rsa_modulus, 0x100);
	}
	else
	{
		memset(access_descriptor_.ncch_rsa_modulus, 0xFF, 0x100);
	}
	...
	{
		Crypto::Sha256((u8*)&access_descriptor_.ncch_rsa_modulus, sizeof(struct sAccessDescriptor) - 0x100, hash);
		safe_call(Crypto::SignRsa2048Sha256(accessdesc_rsa_modulus, accessdesc_rsa_priv_exponent, hash, access_descriptor_.signature));
	}
	else
	{
...
void CxiExtendedHeader::SetFsAccessRights(u64 rights)
{
