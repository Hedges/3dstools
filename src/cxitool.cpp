...
private:
	...
	int MakeRomfs()
	{
		if (args_.romfs_dir)
		{
			...
			romfs_full_size_ = ivfc_.header_size() + romfs_.data_size() + ivfc_.level0_size() + ivfc_.level0_size();
	...
	int MakeExheader()
	{
		...
		safe_call(extended_header_.CreateExheader(NULL, NULL, NULL));
	...
	int MakeHeader()
	{
		...
		*/
		...
		safe_call(header_.CreateHeader(NULL, NULL));
