...
public:
	...
	inline u32 certificate_offset() const { return align(sizeof(struct sCiaHeader), kCiaSizeAlign); }
	inline u32 ticket_size() const { return le_word(header_.ticket_size); }
	inline u32 ticket_offset() const { return align(certificate_offset() + certificate_size(), kCiaSizeAlign); }
	inline u32 title_metadata_size() const { return le_word(header_.title_metadata_size); }
	inline u32 title_metadata_offset() const { return align(ticket_offset() + ticket_size(), kCiaSizeAlign); }
	inline u64 content_size() const { return le_dword(header_.content_total_size); }
	inline u32 content_offset() const { return align(title_metadata_offset() + title_metadata_size(), kCiaSizeAlign); }
