	...
	access_descriptor_size_(0)
{
	...
	memcpy(header_.magic, NCCH_MAGIC, 4);
...
int NcchHeader::CreateHeader(const u8 modulus[0x100], const u8 priv_exponent[0x100])
{
	...
	if (modulus != NULL && priv_exponent != NULL)
	{
		// hash header
		Crypto::Sha256((u8*)header_.magic, sizeof(struct sNcchHeader) - 0x100, hash);
		// sign header
		safe_call(Crypto::SignRsa2048Sha256(modulus, priv_exponent, hash, header_.signature));
	} 
